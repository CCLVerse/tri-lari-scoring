...
#' @export
build_bifactor_model <- function(competency_cols_names=NULL, df=NULL, factor_name=NULL, raters=NULL, regex_pattern=FALSE){

    validate_df(df)
    
    
    model_params <- configure_model_parameters(df=df, raters=raters, competency_cols_names=competency_cols_names, factor_name=factor_name, regex_pattern=TRUE)
    ...
    model <- build_first_level_model(competency_cols_names=competency_cols_names, df=df, factor_name=factor_name, raters=raters, regex_pattern=regex_pattern) 
